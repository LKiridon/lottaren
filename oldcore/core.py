...
def run_draw(seed: str) -> DrawResult:
    ...
    items_sorted = sorted(
        items,
        key=lambda r: (comp.get(int(r['id']), 0), str(r['name']).lower()),
        reverse=True,
    )
    ...
    for it in items_sorted:
        ...
        item_units.extend([iid] * max(1, qty))

    for iid in item_units:
        ...
        if not weight_snapshot:
            # nobody wanted it
            allocations_to_insert.append((run_id, iid, None, json.dumps({}), int(time.time())))
            continue

        winner = weighted_choice(rng, weight_snapshot)
        wins[winner] += 1
        allocations_to_insert.append((run_id, iid, winner, json.dumps(weight_snapshot), int(time.time())))
